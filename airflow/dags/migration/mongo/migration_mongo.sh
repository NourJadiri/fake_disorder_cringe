...
echo "Restoring database: Ingestion_db from BSON file..."
docker exec -i "$MONGO_CONTAINER_NAME" mongorestore --db Ingestion_db /tmp/backup/Ingestion_db
echo "Restoring database: Staging_db from BSON file..."
...
echo "Successfully restored MongoDB databases"