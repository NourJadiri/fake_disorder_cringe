...
docker exec -i "$MONGO_CONTAINER_NAME" mongorestore --db Ingestion_db /tmp/backup/Staging_db
echo "Restoring database: Chadd_staging_db from BSON file..."
docker exec -i "$MONGO_CONTAINER_NAME" mongorestore --db Chadd_ingestion_db /tmp/backup/chadd_staging_db